...
const CheckRate = ({ data }: CheckRateProps) => {
    ...
    const [Old, setOld] = useState<Record<string, number> | null>(null);
    const [change, setChange] = useState<string | null>(null);
    const [changeP, setChangeP] = useState<string | null>(null);
    ...
    const handleFetchOld = async (): Promise<void> => {
        try {
    ...
    const handleChange = () => {
        if (prices && Old) {
            const currentRate = prices[RECEIVE];
            const oldRate = Old[RECEIVE];

            if (currentRate !== undefined && oldRate !== undefined && oldRate !== 0) {
                const calc = currentRate - oldRate;
                const calcp = calc / oldRate;

                setChange(calc.toFixed(4));
                setChangeP((calcp * 100).toFixed(2));
            }
        }
    };
    ...
    return (
        <>
            ...
            <CheckRateOptions
                Open={Old?.[RECEIVE] ?? null}
                Last={prices?.[RECEIVE] ?? null}
                calc={change}
                calcp={changeP}
                send={Send}
                Recieve={RECEIVE}
