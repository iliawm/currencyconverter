...
const Nav = async () => {
    ...
    try {
        ...
        const currentRate = data.rates.EUR
        
        const yesterday = new Date()
        yesterday.setDate(yesterday.getDate() - 1)
        const dateStr = yesterday.toISOString().split('T')[0]
        
        const oldResponse = await fetch(`https://api.frankfurter.app/${dateStr}?from=USD`)
        ...
        const currentPromises = pairs.map((pair: { from: string; to: string }) =>
            fetch(`https://api.frankfurter.app/latest?from=${pair.from}&to=${pair.to}`)
                .then(res => res.json())
                .then(res => ({
                    pair,
                    currentRate: res.rates[pair.to]
                }))
        )
        
        const currentData = await Promise.all(currentPromises)
        
        const oldPromises = pairs.map((pair: { from: string; to: string }) =>
            fetch(`https://api.frankfurter.app/${dateStr}?from=${pair.from}&to=${pair.to}`)
                .then(res => res.json())
                .then(res => ({
                    pair,
                    oldRate: res.rates[pair.to]
                }))
        )
        
        const oldData2 = await Promise.all(oldPromises)
        
        currencyData = currentData.map((current: { pair: { from: string; to: string }; currentRate: number }, index: number) => {
            const old = oldData2[index]
